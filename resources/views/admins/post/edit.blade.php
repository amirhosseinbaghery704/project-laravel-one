@extends('layouts.admin', ['title' => 'Edit Post | ' . $post->title])
        ...
        <form action="{{ route('post.update', ['post' => $post->id]) }}" enctype="multipart/form-data" method="POST">
            ...
            <div class="flex justify-between gap-8">
                <div class="w-3/4">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea 
                            ...
                            class="w-full bg-gray-100 rounded-md p-2 mt-2 mb-1"
                            rows="15"
                            placeholder="Enter title">{{ $post->content }}</textarea>
                        @error('content')
                            <small class="text-red-500 px-3">{{ $message }}</small>
                        @enderror
                    </div>
                ...
                <div class="w-1/4">
                    ...
                    <div class="mb-3">
                        ...
                        <select 
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ $post->categories->pluck('id')->contains($category->id)? 'selected' : '' }}>{{ $category->name }}</option>
                            @endforeach
                        </select>
                        @error('categories')
                            <small class="text-red-500 px-3">{{ $message }}</small>
                    ...
                    <div class="mb-3">
                        <button type="submit" class="w-full bg-green-700 text-white rounded-md p-2 cursor-pointer">Update Post</button>
                    </div>
                </div>
            </div>
        </form>
