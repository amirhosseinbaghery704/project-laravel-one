    ...
    <div>
        <h3 class="text-lg font-semibold text-gray-900 mb-3">Search</h3>
            <form action="{{ route('search') }}">
                ...
                <input name="word" value="{{ request('word') }}" class="w-40 ml-1 bg-transparent focus:outline-none focus:border-none focus:ring-0 outline-none border-none text-xs text-gray-800 placeholder:text-gray-400" type="text" placeholder="Search Yelo">
            </div>
        </form>
    </div>
</div>
    ...
    <div id="recommended-topics-box">
        ...
        <div class="topics flex flex-wrap justify-start">
            @foreach ($popularCategories as $category)
                <a href="{{ route('category', ['category' => $category->id]) }}" class="bg-red-600 text-white rounded-xl px-3 py-1 text-base my-1">{{ $category->name }} <small>({{ $category->posts_count }} posts)</small></a>
            @endforeach
        </div>
    </div>
@endif
